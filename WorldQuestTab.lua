...
function WQT_RewardDisplayMixin:AddRewardByInfo(rewardInfo, warmodeBonus)
    ...
    self:AddReward(
        ...
        rewardInfo.quality,
        rewardInfo.amount,
        rewardInfo.textColor,
        rewardInfo.canUpgrade,
        warmodeBonus
    )
end
...
function WQT_RewardDisplayMixin:AddReward(rewardType, texture, quality, amount, typeColor, canUpgrade, warmodeBonus)
    ...
    if (self.numDisplayed >= WQT.settings.list.rewardNumDisplay) then
        return
    end
...
function WQT_ListButtonMixin:UpdateTime()
    ...
    local _, timeString, color, _, _, category =
        WQT_Utils:GetQuestTimeString(self.questInfo, WQT.settings.list.fullTime)
...
function WQT_ListButtonMixin:Update(questInfo, shouldShowZone)
    ...
    -- Highlight
    local showHighLight =
        self:IsMouseOver() or self.Faction:IsMouseOver() or
...
function WQT_ScrollListMixin:ApplySort()
    ...
    table.sort(
        list,
        function(a, b)
            return SortQuestList(a, b, sortOption)
        end
    )
end
...
function WQT_ScrollListMixin:UpdateQuestList()
    ...
    if (not (flightShown or worldShown)) then
        return
    end
...
function WQT_ScrollListMixin:DisplayQuestList()
    ...
    local shouldShowZone =
        WQT.settings.list.showZone and
...
function WQT_QuestCounterMixin:InfoOnEnter(frame)
    ...
    GameTooltip:AddDoubleLine(
        "API - Addon = Displayed",
        ("|cFFFFFFFF%d - %d = %d|r"):format(questCount, #self.falseCounted, self.numQuests),
        1,
        1,
        1,
        1,
    ...
    for k, i in ipairs(self.falseCounted) do
        ...
        GameTooltip:AddDoubleLine(
            string.format("%s (%s)", info.title, info.questID),
            tagInfo and tagInfo.tagName or "No tag",
            1,
            1,
            1,
            1,
...
function WQT_CoreMixin:TryHideOfficialMapPin(pin)
    if (WQT.settings.pin.disablePoI) then
        return
    end
...
function WQT_CoreMixin:ShowWorldmapHighlight(questID)
    ...
    if (not coords or not mapInfo) then
        return
    end
    ...
    local fileDataID, atlasID, texPercentageX, texPercentageY, textureX, textureY, scrollChildX, scrollChildY =
        C_Map.GetMapHighlightInfoAtPosition(WorldMapFrame.mapID, coords.x, coords.y)
    if (fileDataID and fileDataID > 0) or (atlasID) then
        ...
        if (atlasID) then
            ...
            if textureX > 0 and textureY > 0 then
                ...
                WQT_MapZoneHightlight.Texture:SetPoint(
                    "TOPLEFT",
                    WQT_MapZoneHightlight:GetParent(),
                    "TOPLEFT",
                    scrollChildX,
                    scrollChildY
                )
...
function WQT_CoreMixin:OnLoad()
    ...
    self.dataProvider:RegisterCallback(
        "WaitingRoom",
        function()
    ...
    hooksecurefunc(
        ...
        function()
            ...
            local anchor =
                WorldMapFramePortrait:IsShown() and _V["LIST_ANCHOR_TYPE"].world or _V["LIST_ANCHOR_TYPE"].full
    ...
    hooksecurefunc(
        WorldMapFrame,
        "HandleUserActionToggleQuestLog",
        function()
            if not WorldMapFrame:IsShown() then
                return
            end
    ...
    if (worldMapFilter) then
        ...
        worldMapFilter.ResetButton:HookScript(
            "OnClick",
            function()
                UpdateFilters()
            end
    ...
    hooksecurefunc(
        ...
        "OnTabClick",
        function(self, tab)
            if (not WQT.settings.general.autoEmissary or tab.isEmpty or WQT.settings.general.emissaryOnly) then
                return
            end
    ...
    hooksecurefunc(
        ...
        "AnchorBountyTab",
        function(self, tab)
            if (not WQT.settings.general.bountyCounter) then
                return
            end
            ...
            if (self.searching and not InCombatLockdown()) then
                ...
                if
                    (WQT_GroupSearch.questID and WQT_GroupSearch.title and
                        not (searchString:find(WQT_GroupSearch.questID) or
                            WQT_GroupSearch.title:lower():find(searchString)))
                 then
                    WQT_GroupSearch.Text:SetText(
                        _L["FORMAT_GROUP_TYPO"]:format(WQT_GroupSearch.questID, WQT_GroupSearch.title)
                    )
    ...
    LFGListFrame.EntryCreation:HookScript(
        "OnHide",
        function()
    ...
    hooksecurefunc(
        ...
        function(questID, isFromGreenEyeButton)
    ...
    if LFGParent and LFGParent.StartGroupButton then
        LFGParent.StartGroupButton:HookScript(
            "OnClick",
            function()
                ...
                if
                    (WQT_GroupSearch.questID and WQT_GroupSearch.title and
                        (searchString:find(WQT_GroupSearch.questID) or WQT_GroupSearch.title:lower():find(searchString)))
                 then
                    WQT_GroupSearch.Text:SetText(
                        _L["FORMAT_GROUP_CREATE"]:format(WQT_GroupSearch.questID, WQT_GroupSearch.title)
                    )
    ...
    QuestMapFrame.QuestSessionManagement:HookScript(
        "OnShow",
        function()
...
function WQT_CoreMixin:UpdateWorldMapButton()
    ...
    if alignButton then
        ...
        if framesCount > 0 then
            ...
            WQT_WorldMapContainerButton:SetPoint(
                "TOPRIGHT",
                WorldMapFrame.ScrollContainer,
                "TOPRIGHT",
                topRightButtonPoolXOffset,
                -2
            )
        end
    else
...
function WQT_CoreMixin:ShowHighlightOnMapFilters()
    if (not self.worldMapFilter) then
        return
    end
...
function WQT_CoreMixin:ShouldAllowLFG(questInfo)
    ...
    return tagInfo and tagInfo.worldQuestType and
        not (tagInfo.worldQuestType == Enum.QuestTagType.PetBattle or
            tagInfo.worldQuestType == Enum.QuestTagType.Dungeon or
            tagInfo.worldQuestType == Enum.QuestTagType.Progession or
            tagInfo.worldQuestType == Enum.QuestTagType.Raid)
end
...
function WQT_CoreMixin:ADDON_LOADED(loaded)
    ...
    if (loaded == "Blizzard_FlightMap") then
        ...
        hooksecurefunc(
            ...
            "AddWorldQuest",
            function(frame, info)
                local flightMapTemplate =
                    FlightMapFrame.pinPools[FlightMap_WorldQuestDataProviderMixin:GetPinTemplate()]
...
function WQT_CoreMixin:HideOverlayFrame()
    local blocker = self.Blocker
    if (not blocker.CurrentOverlayFrame) then
        return
    end
