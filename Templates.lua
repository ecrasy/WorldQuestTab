...
local function _AddQuestRewardsToTooltip(tooltip, questID, style)
    ...
    if not style.atLeastShowAzerite then
        local numAddedQuestCurrencies, usingCurrencyContainer, primaryCurrencyRewardInfo =
            QuestUtils_AddQuestCurrencyRewardsToTooltip(questID, tooltip, tooltip.ItemTooltip)
        ...
        if primaryCurrencyRewardInfo then
            local isFirstTimeReward =
                primaryCurrencyRewardInfo.questRewardContextFlags and
                FlagsUtil.IsSet(
                    primaryCurrencyRewardInfo.questRewardContextFlags,
                    Enum.QuestRewardContextFlags.FirstCompletionBonus
                )
            mainRewardIsFirstTimeReputationBonus =
                isFirstTimeReward and
                (C_CurrencyInfo.GetFactionGrantedByCurrency(primaryCurrencyRewardInfo.currencyID) ~= nil) or
                false
    ...
    if (honorAmount > 0) then
        GameTooltip_AddColoredLine(
            tooltip,
            BONUS_OBJECTIVE_REWARD_WITH_COUNT_FORMAT:format(
    ...
    if
        numQuestRewards > 0 and
            (not style.prioritizeCurrencyOverItem or C_QuestInfoSystem.HasQuestRewardCurrencies(questID))
     then
        if style.fullItemDescription then
            ...
            if numItems > 1 then
                text =
                    ...
                    texture,
                    HIGHLIGHT_FONT_COLOR:WrapTextInColorCode(numItems),
                    name
                )
    ...
    -- spells
    if
        not tooltip.ItemTooltip:IsShown() and
            EmbeddedItemTooltip_SetSpellByFirstQuestReward(tooltip.ItemTooltip, questID)
     then
        showRetrievingData = true
    end
    ...
    if style.atLeastShowAzerite and not hasAnySingleLineRewards and not tooltip.ItemTooltip:IsShown() then
        local numAddedQuestCurrencies, usingCurrencyContainer =
            QuestUtils_AddQuestCurrencyRewardsToTooltip(questID, tooltip, tooltip.ItemTooltip)
        if (numAddedQuestCurrencies > 0) then
            hasAnySingleLineRewards = not usingCurrencyContainer or numAddedQuestCurrencies > 1
    ...
    if
        style.showFirstTimeRepRewardNotice and
            (mainRewardIsFirstTimeReputationBonus or secondaryRewardsContainFirstTimeRepBonus)
     then
        ...
        local noticeText =
            mainRewardIsFirstTimeReputationBonus and QUEST_REWARDS_IS_ONE_TIME_REP_BONUS or
...
function WQT_Utils:AddQuestRewardsToTooltip(tooltip, questID, style)
    ...
    if
        (GetQuestLogRewardXP(questID) > 0 or C_QuestInfoSystem.HasQuestRewardCurrencies(questID) or
...
function WQT_Utils:ShowQuestTooltip(button, questInfo, style)
    ...
    if (not style.hideObjectives) then
        ...
        for objectiveIndex = 1, numObjectives do
            local objectiveText, objectiveType, finished =
                GetQuestObjectiveInfo(questInfo.questID, objectiveIndex, false)

            if (objectiveText and #objectiveText > 0) then
                local objectiveColor = finished and GRAY_FONT_COLOR or HIGHLIGHT_FONT_COLOR
                GameTooltip:AddLine(
                    QUEST_DASH .. objectiveText,
                    objectiveColor.r,
                    objectiveColor.g,
                    objectiveColor.b,
                    true
    ...
    if (questInfo.reward.type == WQT_REWARDTYPE.missing) then
        ...
        if
            ((questInfo.reward.type == WQT_REWARDTYPE.equipment or questInfo.reward.type == WQT_REWARDTYPE.weapon) and
                GameTooltip.ItemTooltip:IsShown())
         then
            if IsModifiedClick("COMPAREITEMS") or C_CVar.GetCVarBool("alwaysCompareItems") then
                ...
                if
                    GameTooltip.ItemTooltip.Tooltip:GetRight() + totalWidth < GetScreenWidth() and
                        ShoppingTooltip1:IsShown()
                 then
...
function WQT_Utils:GetQuestLogInfo(list)
    ...
    local color =
        questCount >= maxQuests and RED_FONT_COLOR or
...
function WQT_Utils:QuestIsVIQ(questInfo)
    ...
    if (questInfo:IsQuestOfType(WQT_QUESTTYPE.threat)) then
        return WQT.settings.general.filterPasses.threat
    end
    if (questInfo:IsQuestOfType(WQT_QUESTTYPE.combatAlly)) then
...
function WQT_Utils:UpdateColor(colorID, r, g, b, a)
    ...
    if (type(r) == "string") then
        local hex = r
        a, r, g, b =
            ExtractColorValueFromHex(hex, 1),
            ExtractColorValueFromHex(hex, 3),
            ExtractColorValueFromHex(hex, 5),
