...
local function ShouldShowPin(
    questInfo,
    mapType,
    settingsZoneVisible,
    settingsPinContinent,
    settingsFilterPins,
    isFlightMap)
    ...
    if (settingsFilterPins and not questInfo.passedFilter) then
        return false
    end
    ...
    if (mapType == Enum.UIMapType.Continent) then
        ...
        if
            (settingsPinContinent == _V["ENUM_PIN_CONTINENT"].tracked and
                not C_QuestLog.GetQuestWatchType(questInfo.questID))
         then
            return false
        end
    ...
    if (mapInfo.mapType >= Enum.UIMapType.Continent) then
        for k, questInfo in ipairs(WQT_WorldQuestFrame.dataProvider:GetIterativeList()) do
            ...
            if (wqp.focusedQuestID) then
                officialShow =
                    C_QuestLog.IsQuestCalling(wqp.focusedQuestID) and
...
function WQT_PinDataProvider:FixOverlaps(canvas)
    if (not self.enableNudging or not canvas) then
        return
    end
...
function WQT_PinMixin:OnLoad()
    ...
    self.iconPool =
        ...
        "FRAME",
        self,
        "WQT_MiniIconTemplate",
        function(pool, iconFrame)
            iconFrame:Reset()
        end
    )
    self.icons = {}
...
function WQT_PinMixin:UpdateVisuals()
    ...
    if (not questInfo:DataIsValid()) then
        return
    end
    ...
    local showTypeIcon =
        WQT_Utils:GetSetting("pin", "typeIcon") and
        (not tagInfo or (questType and questType > 0 and questType ~= Enum.QuestTagType.Normal) or
            questInfo:IsSpecialType())
    ...
    if (WQT_Utils:GetSetting("pin", "timeIcon")) then
        local start, total, timeLeft, seconds, color, timeStringShort, timeCategory =
            WQT_Utils:GetPinTime(self.questInfo)
...
function WQT_PinMixin:OnUpdate(elapsed)
    ...
    if (self.isExpired or self.updateTime < self.updateInterval) then
        return
    end
...
function WQT_PinMixin:UpdatePlacement(alpha)
    ...
    parentScaleFactor =
        parentScaleFactor * Lerp(self.startScale, self.endScale, Saturate(self.scaleFactor * zoomPercent))
...
function WQT_PinMixin:FadeOut()
    if (self.fadeInAnim:IsPlaying()) then
        self.fadeInAnim:Stop()
    end
    self.isFaded = true
